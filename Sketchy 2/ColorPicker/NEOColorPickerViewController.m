...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        int colorCount = NEOColorPicker4InchDisplay() ? 24 : 16;
        for (int i = 0; i < colorCount; i++) {
            UIColor *color = [UIColor colorWithHue:i / (float)colorCount saturation:0.75 brightness:1.0 alpha:1.0];
            [_colorArray addObject:color];
        }
        
        colorCount = NEOColorPicker4InchDisplay() ? 8 : 8;
        for (int i = 0; i < colorCount; i++) {
...
//    [NTTAppDefaults setupSecondaryButton:self.buttonHueGrid];
    ...
    CGRect frame = CGRectMake(20, 20, 230, 40);
    ...
    layer.frame = CGRectMake(20, 20, 230, 40);
    ...
    for (int i = 0; i < colorCount && i < _colorArray.count; i++) {
        ...
        int column = i % 4;
        int row = i / 4;
        layer.frame = CGRectMake((column * 72.5), row * 46, 62.5, 40);
...
- (void) colorGridTapped:(UITapGestureRecognizer *)recognizer {
    ...
    int row = (int)((point.y -8) / 46);
    int column = (int)((point.x -8) / 72.5);
